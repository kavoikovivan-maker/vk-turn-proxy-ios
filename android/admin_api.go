...
//
// Вместо deploy-пароля владельца в заголовке X-Admin-Password — отдельный
// Bearer-токен, сгенерированный при деплое (32 случайных байта). Сервер хранит
// только sha256-хэш токена; сравнение constant-time; при 5 неудачных попытках
// за минуту источник блокируется на 5 минут. Сам API слушается только по
// HTTPS с pinned-сертификатом (см. main() и deploy.sh).
