...
func handleConnRaw(ctx context.Context, clientConn net.Conn, router *rawRouter) {
	...
	if isGetConf {
		...
		if valid && isGenPass {
			// Device уже занят другим профилем в raw-режиме — не даём
			// "перехватить" устройство чужим паролём.
		...
		// пароль здесь не проверяется на уровне DTLS — авторизацию уже подтвердил
		// obfs WRAP-ключ (без верного пароля пакет не прошёл бы аутентификацию
		// на уровне obfs). Но binding устройства к профилю перепроверяем: без
		// этого воркер с чужим (но валидным по obfs) паролем мог бы
		// зарегистрировать сессию на чужой IP.
		dbMutex.Lock()
		isMainPass := password != "" && password == db.MainPassword
		entry, isGenPass := db.Passwords[password]
	...
	for {
		...
		// Анти-спуфинг: uplink-пакет обязан быть IPv4 с src IP, равным
		// назначенному устройству. Иначе вредоносный клиент мог бы отправлять
		// пакеты от имени других адресов туннеля.
...
// ─── Challenge-response аутентификация профильного API ──────────────────────
//
// Раньше клиент отправлял пароль открыто в GET-параметре /api/profile/status
// — он попадал в access-логи nginx/прокси и history браузера. Теперь обмен
// двухшаговый: клиент запрашивает одноразовый nonce, затем доказывает знание
// пароля HMAC-подписью (action\ndevice_id\nnonce), не передавая сам пароль.
...
// profileKeyID — публичный идентификатор профиля: sha256 от константной
// соли + пароль. Позволяет серверу найти профиль по key_id без передачи
// пароля; обратное восстановление пароля из key_id невозможно.
...
func main() {
	...
	// Запуск HTTP Control API (только профильные challenge-роуты; админ-API
	// переехал на отдельный HTTPS-листенер с pinned-сертификатом)
	go func() {
...
// ─── Replay-защита (анти-replay окно) ───────────────────────────────────────
//
// Зеркало клиентской реализации (go_client/obfs.go): перехваченный и
// переотправленный obfs-пакет (например, GETCONF/AUTH-фаза) не должен быть
// принят повторно. Окно — по расширенному RTP-timestamp + точный дедуп по
// 12-байтному nonce.
...
func (c *wrapPacketConn) ReadFrom(p []byte) (int, net.Addr, error) {
	...
	if uErr != nil {
		...
		if uErr2 == nil {
			// Ключ сменился — окно replay начинаем заново (новая сессия obfs).
