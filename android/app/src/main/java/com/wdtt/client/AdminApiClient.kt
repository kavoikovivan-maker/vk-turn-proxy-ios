...
/**
 * Клиент к admin HTTPS API сервера (см. server.go/admin_api.go, /admin/passwords...) —
 * то же управление паролями/устройствами, что уже умеет Telegram-бот, просто
 * второй способ дёрнуть ту же логику. Авторизация — Bearer-токен, выданный при
 * деплое; TLS-сертификат сервера пинится по SHA-256 (self-signed, без CA).
 */
object AdminApiClient {
    ...
    // Регистрирует pin сертификата (base64(sha256(DER)), формат "sha256/...")
    // для хоста. Вызывается после успешного деплоя (DeployTab парсит пин из
    // вывода deploy.sh) и перед запросами (AdminTab вызывает с сохранённым пином).
    ...
    private fun clientForHost(host: String): OkHttpClient {
        ...
        return clients.getOrPut(cacheKey) {
            ...
            OkHttpClient.Builder()
                ...
                .hostnameVerifier { _, _ -> true } // подлинность гарантирует pin серта
