...
#  Версия: 3.3  |  Дата: 2026-08-24
#  NAT:  MASQUERADE через iptables
#  WG:   порт 56001 (не конфликтует с существующим WG на 51820)
#  DTLS: порт 56000
#  ADMIN: порт 56002 TCP — админ-API по HTTPS с pinned-сертификатом
# ==============================================================================
set -uo pipefail
...
setup_nat_and_firewall() {
    ...
    fw_add_input_tcp "$ADMIN_PORT"   # 56002 — админ-API по HTTPS
...
# Настройка защищённой админ-панели: токен из /tmp/wdtt-admin.token (загружен
# приложением при деплое) копируется в конфиг-директорию, генерируется
# self-signed TLS-сертификат (если ещё нет), на stdout печатается pin
# сертификата — приложение парсит его из вывода и сохраняет для pinned-TLS.
setup_admin_tls() {
