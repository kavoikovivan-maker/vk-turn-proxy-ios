...
//
// Злоумышленник может перехватить обфусцированный UDP-пакет и отправить его на
// сервер заново (replay). Для raw-режима это опасно: повтор пакета с
// GETCONF/данными внутри туннеля может смутить сервер. Защита — скользящее
// окно по времени RTP-заголовка (расширенный 64-битный timestamp) плюс точный
// дедуп по 12-байтному nonce (SSRC+Seq+TS, тот же, что AEAD-nonce).
