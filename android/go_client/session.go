...
const (
	...
	// keepaliveInterval: 10с вместо прежних 1с. Агрессивный 1-секундный
	// keepalive на каждом из 18-108 relay-сокетов сессии будил радиомодуль
	// телефона десятки раз в минуту даже при простое — это заметно жрало
	// батарею (см. release notes v1.4.1 апстрима). TURN permissions по умолчанию
	// живут 5 минут, NAT-маппинг дольше 10с — 10с держит всё тёплым без
	// лишних пробуждений.
...
func RunSession(
	...
	// Канал ошибки транспорта: Writer/Reader кладут сюда первую ошибку, чтобы
	// RunSession вернула её наверх в group.go — там по тексту ("broken pipe",
	// "connection reset", ...) принимается решение о быстром перезапуске.
