...
// WorkerGroup). Тот же порядок величины, что у free-turn-proxy; 100ms вместо
// прежних 200ms — старт группы из 9 воркеров укладывается в ~1с, при этом
// квота VK (error 486) по-прежнему не выжигается: аллокации идут по одной.
...
func WorkerGroup(
	...
	for i, wid := range workerIDs {
		...
		// Stagger: 75мс между воркерами — старт группы быстрее, а реальный
		// разнос Allocate-запросов всё равно держит общий allocateTicker.
		...
		go func(wid int, delay time.Duration) {
			...
			for {
				...
				if sessErr != nil {
					...
					// Обрыв транспорта (мёртвый relay/сеть моргнула) — не надо
					// ждать общие 5-15с, воркер просто пересоздаёт сессию.
